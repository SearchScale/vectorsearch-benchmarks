...
while getopts ":-:" opt; do
    case $OPTARG in
        ...
        help)
            ...
            echo "  IVF-PQ Solr: use e.g. --sweeps solr-sweeps-ivfpq-wiki1m-smoke.json (--mode solr)."
...
if [ "$RUN_BENCHMARKS" = "true" ]; then
    ...
    for SWEEP_DIR in "$CONFIGS_DIR"/*; do
	    ...
	    for CONFIG_FILE in $(find "$SWEEP_DIR" -name "*.json" | awk -F'[/-]' '{split($NF, a, "."); gsub("ef", "", a[1]); print $(NF-2)"-"$(NF-1), a[1], $0}' | sort -k1,1 -k2,2n | cut -d' ' -f3); do
