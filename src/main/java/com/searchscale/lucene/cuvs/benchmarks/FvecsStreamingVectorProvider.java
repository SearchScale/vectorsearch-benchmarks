...
public class FvecsStreamingVectorProvider implements VectorProvider {
  private static final Logger log =
      LoggerFactory.getLogger(FvecsStreamingVectorProvider.class.getName());
  ...
  public FvecsStreamingVectorProvider(String filePath, int maxVectors) throws IOException {
    ...
    if (isCompressed) {
      ...
      try (RandomAccessFile raf = new RandomAccessFile(filePath, "r")) {
        ...
        this.vectorCount =
            maxVectors > 0 ? Math.min((int) totalVectors, maxVectors) : (int) totalVectors;
      }
    }

    log.info(
        "FvecsStreamingVectorProvider initialized: {} vectors, {} dimensions",
        vectorCount,
        dimension);
  }
