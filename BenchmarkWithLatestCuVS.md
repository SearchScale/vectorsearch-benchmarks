...
## Benchmark using latest cuVS and cuVS-Lucene

We can use part of this codebase to benchmark `cuVS-Lucene`.
`cuVS Lucene` depends on `cuVS` and `cuVS-Java` for its working.

It is recommended to benchmark using the latest available code in `cuVS` and `cuVS-Lucene`.
Latest here means using the `main` branch for both `cuVS` and `cuVS-Lucene`.

## Prerequisites

### Code
Get `cuVS`, `cuVS-Lucene`, and `vectorsearch-benchmarks` codebase
```sh
mkdir code && cd code
git clone --branch main https://github.com/rapidsai/cuvs.git
git clone --branch main https://github.com/rapidsai/cuvs-lucene.git
git clone --branch benchmark-with-latest-cuvs https://github.com/SearchScale/vectorsearch-benchmarks.git
```
...
Extract maven, cmake, and java downloaded above and update the `PATH` and `LD_LIBRARY_PATH` variables in `.bashrc` like the following.
```sh
export PATH="$CUDA_HOME/bin:$MAVEN_HOME/bin:$CMAKE_HOME/bin:$JAVA22_HOME/bin:$PATH"
export LD_LIBRARY_PATH={REPLACE_WITH_YOUR_CUVS_REPO_DIR}/cpp/build/c:$LD_LIBRARY_PATH
```

Confirm the versions like the following:
```sh
...
We need to build `cuVS` following by `cuVS-Lucene` and then `vectorsearch-benchmarks`.
...
```sh
./run_sweep.sh --data-dir /home/narang/data/wikipedia/1Mx768 --datasets trials/0/datasets_trial.json --sweeps trials/0/sweeps_trial_cag-hnsw.json --configs-dir configs --results-dir results --run-benchmarks
```

The sweep configuration for Wikipedia 10Mx768 dataset is available in the `trials/1` folder.
